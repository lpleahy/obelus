...
T.it("render-markdown bullet twin carries the bubble bg (overlay markers must not box dark)", function()
  T.fresh({ render = { transparent = false } })
  local thread = require("obelus.thread")
  thread.setup_highlights()
  thread.render_md_harmonize()
  local tw = vim.api.nvim_get_hl(0, { name = "Obelus_RenderMdBullet" })
  local bubble = vim.api.nvim_get_hl(0, { name = "ObelusReplyBg" })
  T.ok(tw.bg ~= nil, "bullet twin has a bg — fg-only lets the dark window bg show through the overlay")
  T.eq(tw.bg, bubble.bg, "…and it is the agent bubble's seamless bg")
end)
